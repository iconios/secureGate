...
2. Call the unverified manager accounts deletion service
3. Call the unpaid estate entries deletion service
4. Call the unpaidpayment entries deletion service
...
import { DeleteUnverifiedManagerAccountsService } from '../managers/delete.unverified.accounts.service.js';
  ...
  try {
    ...
    if (!apiKey || typeof apiKey !== 'string') {
      return res
        .status(400)
        .json(
          errorResponseHelper('Api key is required', 'API_KEY_NOT_PROVIDED', 'Api key is required'),
        );
    }
    ...
    if (!apiKeyAuthResult.success) {
      if (
        apiKeyAuthResult.error?.code === 'MAINTENANCE_API_KEY_NOT_CONFIGURED' ||
        'UNKNOWN_ERROR'
      ) {
    ...
    // 2. Call the unverified manager accounts deletion service
    const unverifiedManagersResult = await DeleteUnverifiedManagerAccountsService();
    if (!unverifiedManagersResult.success) {
      return res.status(500).json(unverifiedManagersResult);
    }
    ...
    // 4. Call the unpaid payment entries deletion service
    ...
    // 5. Send the response back to the client
    dbMaintenanceLogs.info('Database cleanup tasks completed');
    return res.status(200).json(
      successResponseHelper('Database cleanup tasks completed', {
        unverifiedManagersResult,
    ...
    dbMaintenanceLogs.error('Unexpected controller error during database cleanup', {
