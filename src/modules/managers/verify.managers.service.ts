...
2. Check if the email and token match the records in the database and are still valid (not expired).
3. If valid, update the user's status to "verified" in the database.
4. If unvalid, send an appropriate error response (e.g., "Invalid token" or "Token expired").
...
const VerifyManagerService = async (verifyManagerData: VerifyManagerData) => {
  ...
  try {
    // 1. Accept and validate the verification request (e.g., email, token).
    const { email, token } = verifyManagerDataSchema.parse(verifyManagerData);
    managerLogs.info('Processing verification request', { email });
    ...
    const { data: verificationRequest, error: verificationError } = await supabaseAdmin
      ...
      .select('id, email, purpose, token_hash, status, token_expires_at')
      .eq('email', email)
    ...
    // 3. If valid, update the user's status to "verified" in the database.
    if (verificationRequest.token_hash) {
      const isTokenMatch = await compareString(token, verificationRequest.token_hash);
      if (isTokenMatch) {
        managerLogs.info('Verification token validated successfully', { email });
        ...
        managerLogs.error('Invalid verification token', { email });
        return errorResponseHelper(
    ...
    managerLogs.error('Unexpected error occurred during verification', { emailprocessing, error });
